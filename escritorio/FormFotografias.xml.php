<?php
	...
	if(is_user_logged_in() && current_user_can('administrator'))
	{
		...
		$_SESSION['pA'] = $pc -1;
		$_SESSION['pS'] = $pc +1;
		$_SESSION['pT'] = $wpdb->get_var("select count(*) from galeria g where g.cuil_dipu='".$cuil."' and g.type='fotografia' ".$w);
		...
		$sql = "select id,nombre,descripcion,date_format(fecha,'%d-%m-%Y') dia,date_format(fecha,'%h:%i:%s') hora,estado from galeria g where g.cuil_dipu='".$cuil."' and g.type='fotografia' ".$w." order by g.orden desc, g.id desc limit ".$li.",".$le.";";
